...
local function vapeGithubRequest(scripturl, isImportant)
    ...
    local suc, res
    local url = (scripturl == 'Universal.lua' or scripturl == 'VWUniversal.lua'or scripturl ==  'GuiLibrary.lua' or scripturl == 'MainScript.lua' or scripturl == 'CustomModules/CE6872274481.lua') and"https://raw.githubusercontent.com/FTMteam/xV4/" or "https://raw.githubusercontent.com/VapeVoidware/VoidwareBakup/"
...
local function pload(fileName, isImportant, required)
    ...
    if (not suc) then 
        if isImportant then
            if (not string.find(string.lower(err), "x v4 already injected")) and (not string.find(string.lower(err), "rise already injected")) then
                task.spawn(function()
